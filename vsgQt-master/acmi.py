...
class Acmi:
    ...
    # 解析事件属性
    def _parse_Event_property(self, fields: list):
        print('事件', fields)
        global obj
        for field in fields[1:]:  # skip objid (0)
            (prop, val) = field.split('=', 1)
            if prop == 'Event':
                event_list = val.split('|')
                event_type = event_list[0]
                #判断事件类型
                #Messag: 消息提示各个模型触发xxx事件 最后一位是消息文本
                if event_type == 'Message':
                    #取出模型对象
                    obj_list = event_list[1:-1]
                    obj_text = event_list[-1]
                    obj =  {
                        "type":'Event',
                        'class':'Message',
                        "obj_list": obj_list,
                        "obj_text": obj_text,
                        'time': self.cur_reftime,
                    }
                elif event_type == 'lineMode':
                    obj = {
                        "type":"Event",
                        "class": "lineMode",
                        "id":event_list[1],
                        "modelID1":event_list[2],
                        "modelID2": event_list[3],
                        'time': self.cur_reftime,
                    }
                elif event_type == 'destory':
                    obj = {
                        'type':'Event',
                        'class':'destory',
                        'typeModel': event_list[1],
                        'id': event_list[2],
                        'time': self.cur_reftime,
                    }
                elif event_type == 'picMode':
                    obj = {
                        'type':'Event',
                        'class':'picMode',
                        'id': event_list[1],
                ...
                elif event_type == 'sensorControl':
                    obj = {
                        'type':'Event',
                        'class':'sensorControl',
                        'id': event_list[2],
                        'modelID': event_list[1],
                        'sensor': event_list[3].lower(),
                        'state': event_list[4].lower(),
                        'time': self.cur_reftime,
                    }
            if prop == 'Color':
                (prop, val) = field.split('=', 1)
    ...
    #通过点集获取经纬度列表
    def _get_points(self,plot_list:list) -> list:
        #取出点数据
        length = 3 * ((len(plot_list) - 1) // 3)
        point_list = plot_list[1:length+1]
        point_list = [item if item else '0' for item in point_list]
        #将数据转化为经纬度点存入数组
        n = length // 3
        points = []
        for i in range(n):
            longitude = point_list[3 * i]
            latitude = point_list[3 * i + 1]
            height = point_list[3 * i + 2]
            point = {
                'lon':longitude,
                'lat':latitude,
                'height':height
    ...
    # 解析标会属性
    #Plot: 对地球进行绘制图形,类型在Plot=plot_type,数据通过|进行分割 |后无数据自动转化为0
    def _parse_Plot_property(self, fields: list):
        # print('事件', fields)
        for field in fields[1:]:  # skip objid (0)
            (prop, val) = field.split('=', 1)
            if prop == 'Plot':
                ...
                #Point : 绘制点 
                if plot_type == 'Point':
                    points = self._get_points(plot_list)
                    return {
                        "type":'Plot',
                        'plot_type':'point',
                        "point_list": points,
    ...
    def _parse_global_property(self, fields: list):
        for field in fields[1:]:  # skip objid (0)
            ...
            # 添加仿真状态属性
                ...
                event = self._parse_Event_property(fields)
                return event
            elif prop == "Plot":
                plot = self._parse_Plot_property(fields)
                return plot
    ...
    def line_do_parse(self,rawline):
        if rawline.startswith('FileType='):
            self.file_type = rawline[len('FileType='):].strip()
            #初始化时候Chanel置空
            self.Chanel = ''
        ...
        if line.startswith('-'):
            ...
            # print(obj_id, fields)
            if obj_id == '0' or obj_id == 0:
                state = self._parse_global_property(fields)
                #添加自定义标识 仿真开始返回1 结束返回0
                #str类型 -> 启动与结束 后续开发统一使用dict
                if type(state) == str:
                    return [-1,state,self.cur_reftime]
                if type(state) == dict:
                    if state['type'] == 'Event':
                        return [2,state]
                    elif state['type'] == 'Plot':
                        return [2,state]
                    else:
                        return [2,state]
                return [0]
            else:
                self._parse_object_property(obj_id, self.cur_reftime, fields)
                return [1,obj_id]
    ...
    def export_csv(self, filepath, remove_empty=True, export_obj_ids = None):
        # 如果目录不存在，则创建目录
